...
export function ConfiguracionMobile() {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 pb-8">
      <div className="bg-white px-4 py-4 shadow-sm border-b">
      ...
      <div className="flex-1 space-y-4 px-4 py-6">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="bg-blue-50 px-4 py-3 border-b border-blue-100">
            <h2 className="font-semibold text-blue-800">Datos Personales</h2>
          </div>
          <div className="p-4">
            <form onSubmit={formPerfil.handleSubmit(onPerfilSubmit)} className="space-y-4">
              <div>
                ...
                {formPerfil.formState.errors.nombre && (
                  <p className="text-red-500 text-xs mt-1">{formPerfil.formState.errors.nombre.message}</p>
                )}
              </div>
              ...
              <div>
                ...
                {formPerfil.formState.errors.apellido && (
                  <p className="text-red-500 text-xs mt-1">{formPerfil.formState.errors.apellido.message}</p>
                )}
              </div>
              ...
              <div className="pt-2">
                ...
                >
                  {isActualizandoPerfil ? 'Guardando...' : 'Guardar Datos'}
                </Boton>
                {perfilExito && <p className="text-green-600 text-center text-sm font-medium mt-2">¡Perfil actualizado!</p>}
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
          <div className="bg-blue-50 px-4 py-3 border-b border-blue-100">
            <h2 className="font-semibold text-blue-800">Seguridad</h2>
          </div>
          <div className="p-4">
            <form onSubmit={formContrasena.handleSubmit(onContrasenaSubmit)} className="space-y-4">
              <div>
              ...
              <div className="pt-2">
                <Boton 
                  ...
                  className="w-full py-2.5 rounded-lg"
                >
                  {isModificandoContrasena ? 'Actualizando...' : 'Cambiar Contraseña'}
                </Boton>
              </div>
              {contrasenaExito && (
                <div className="p-3 bg-green-50 border border-green-200 text-green-700 rounded-lg text-sm mt-3 text-center">
